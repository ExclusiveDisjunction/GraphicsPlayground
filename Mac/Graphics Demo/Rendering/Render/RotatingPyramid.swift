...
final class RotatingPyramidRenderer : NSObject, MTKViewDelegate {
    ...
    init(_ device: MTLDevice) throws(MissingMetalComponentError) {
        ...
        self.loc = float4x4(translation: .init(0, 0, 0))
    ...
    func draw(in view: MTKView) {
        ...
        let totalLoc = float4x4(rotationY: Float(frame) / 60 / (.pi / 8 )) * loc;
        
        memcpy(buffer.contents(), [totalLoc], MemoryLayout<float4x4>.stride)
        ...
        renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: mesh.count, instanceCount: 1)
